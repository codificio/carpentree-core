...
class ProductForm extends Form {
  ...
  handleNewImageForeground = async () => {
    ...
    await Http.post(apiUrl + "/product/" + data.id + "/images/foreground/" + imageForeground_index);
  ...
  handleRemoveImage = async () => {
    const { data, imageForeground_index } = this.state;
    await Http.post(apiUrl + "/product/" + data.id + "/images/delete/" + imageForeground_index);
    this.setState({ imageForeground_index: 0})
    const productId = this.props.match.params.id;
    this.productLoad(productId);
  }
  ...
  handleDropThumbnail = async (dragIndex, hoverIndex) => {
    ...
    const { data } = this.state;
    await Http.post(apiUrl + "/product/" + data.id + "/images/exchange/" + dragIndex + "|" + hoverIndex);
              ...
              <div className="row">
                    ...
                    {data.media &&
                      data.media.map((img, img_index) => (
                        <ThumbnailDraggable 
                          ...
                          img_index={img_index} 
                          onClick={() => this.handleChangeimageForeground(img, img_index)}
                ...
                <div className="col-sm-12 col-md-10">
                  <div className="squareBox boxWithBorder">
                    <div>
                      {imageForeground !== "" && imageForeground !== noImageUrl && (
                        <div
                          style={{
                            position: "absolute",
                            zIndex: "100",
                      ...
                      >
                      { imageForeground_index == 0 ? <FaBookmark className="text-primary" /> : <FaBookmark />}
                      </div>
                      )}
                      <img
                        src={imageForeground}
